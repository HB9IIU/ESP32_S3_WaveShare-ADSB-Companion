...
### `src/satellite_map.h` / `src/satellite_map.cpp`

ESP32-S3 direct ESRI World Imagery tile renderer. The ESP32 calculates the Web Mercator XYZ tiles for the configured center coordinate and radius, downloads each ESRI JPEG tile one by one, validates it, and draws the relevant scaled portion directly to the 800x480 LovyanGFX display. It does not use a local map server and does not stitch a full raw framebuffer in RAM.
...
Map defaults live in `src/myconfig.h`:

```cpp
#define MAP_CENTER_LAT 46.46651
#define MAP_CENTER_LON 6.85534
#define MAP_RADIUS_KM  150.0f
```

Current boot flow: show splash, connect WiFi, sync time, calculate the required ESRI tiles, download/decode/draw them, then leave the satellite map on screen.
